...
public class SimulationRunner {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (PrintWriter statsWriter = new PrintWriter(new FileWriter("simulation_stats.csv"));
            ...
            statsWriter.println("Time,Crusher Utilization,Average Crusher Queue,Excavator 1 Utilization,Excavator 1 " 
                + "Average Queue,Excavator 2 Utilization,Excavator 2 Average Queue,Excavator 3 Utilization,Excavator 3 Average Queue");
            ...
            sim.go(simulationTime, (time) -> {
                ...
                if (time >= nextLogTime[0]) {
                    ...
                    double exc1Queue = systemNet.waitExcavatorPlaces.get(0).getMean() 
                        + systemNet.waitExcavatorPlaces.get(1).getMean();

                    double exc2Util = 1.0 - systemNet.freeExcavators.get(1).getMean();
                    double exc2Queue = systemNet.waitExcavatorPlaces.get(2).getMean() 
                        + systemNet.waitExcavatorPlaces.get(3).getMean();

                    double exc3Util = 1.0 - systemNet.freeExcavators.get(2).getMean();
                    double exc3Queue = systemNet.waitExcavatorPlaces.get(4).getMean() 
                        + systemNet.waitExcavatorPlaces.get(5).getMean();
